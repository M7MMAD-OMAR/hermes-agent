...
export function formatIdentityDetails(identity: CompactIdentity, rect?: IdentityRect): string[] {
  ...
  // The markup upstream rendered inline in pack.ts. Kept here so one helper
  // owns everything an agent needs to locate the element in source.
  if (identity.html) {
    lines.push(`  HTML: ${identity.html}`)
  }
