...
export const ru = defineLocale({
  sessionImport: {
    ...
    chooseHelp: 'Выберите сессию, чтобы прочитать историю перед импортом в Sbar Rafiq.',
    ...
    you: 'Вы',
    snapshot: 'Этот разговор уже есть в Sbar Rafiq. Откройте существующую копию, чтобы продолжить.',
    copyNotice:
      'Копируется текст разговора. Исходные файлы не меняются. Вывод инструментов и рассуждения не переносятся.',
    importing: 'Импорт…',
    open: 'Открыть в Sbar Rafiq',
    continue: 'Продолжить в Sbar Rafiq',
  ...
  boot: {
    ready: 'Sbar Rafiq Desktop готов',
    desktopBootFailedWithMessage: message => `Не удалось запустить приложение: ${message}`,
    steps: {
      connectingGateway: 'Подключение к шлюзу',
      loadingSettings: 'Загрузка настроек Sbar Rafiq',
      loadingSessions: 'Загрузка последних сеансов',
      retryingRemoteBackend: 'Переподключение к удалённому бэкенду Sbar Rafiq…',
      startingDesktopConnection: 'Запуск подключения приложения',
      startingHermesDesktop: 'Запуск Sbar Rafiq Desktop…'
    },
    errors: {
      backgroundExited: 'Фоновый процесс Sbar Rafiq завершён.',
      backgroundExitedDuringStartup: 'Фоновый процесс Sbar Rafiq завершился при запуске.',
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'Ваш бэкенд Sbar Rafiq старше этой сборки приложения и может работать некорректно. Обновите их, чтобы они совпали.',
    installMethodUnsupportedTitle: 'Неподдерживаемый способ установки',
    updateHermes: 'Обновить Sbar Rafiq',
  ...
  settings: {
    ...
    resetConfirm: 'Сбросить все настройки к значениям Sbar Rafiq по умолчанию?',
    ...
    notifications: {
      ...
      focusedHint: 'Оповещения о завершении срабатывают только пока Sbar Rafiq в фоне.',
      kinds: {
        approval: {
        ...
        input: {
          label: 'Нужен ввод',
          description: 'Sbar Rafiq задал вопрос или требует пароль/секрет.'
        },
        turnDone: {
          label: 'Ответ готов',
          description: 'Ход завершился, пока Sbar Rafiq был в фоне.'
        },
        turnError: {
          label: 'Ход не удался',
        ...
        plugin: {
          ...
          description: 'Плагин приложения отправил уведомление, пока Sbar Rafiq был в фоне.'
        }
      },
      ...
      completionSoundDesc:
        'Воспроизводится, когда ход агента завершён. Выберите пресет и прослушайте здесь. У подтверждений, вопросов, ошибок и завершённых фоновых задач свой короткий сигнал, с тем же отключением звука.',
    ...
    appearance: {
      ...
      reactionsDesc: 'Эмодзи-тапбеки в стиле iMessage — реагируйте на сообщения, и Sbar Rafiq сможет реагировать на ваши.',
      ...
      pet: {
        title: 'Питомец',
        intro:
          'Заберите анимированного питомца из petdex, который парит над приложением и реагирует на действия Sbar Rafiq — «бегает», пока выполняются инструменты, радуется успеху и хмурится при ошибках.',
        restartHint:
          'Питомцам нужен быстрый перезапуск — текущее приложение запустилось до появления этой функции. Выйдите из Sbar Rafiq и откройте снова, затем вернитесь сюда.',
        on: 'Вкл',
        off: 'Выкл',
        scaleTitle: 'Размер',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      agent: {
        ...
        maxTurns: 'Верхний предел ходов с вызовами инструментов, после которого Sbar Rafiq останавливает запуск.'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Максимальное число символов, которые Sbar Rafiq может прочитать из одного запроса к файлу.',
      approvals: {
        mode: 'Как Sbar Rafiq обрабатывает команды, требующие явного подтверждения.',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Когда Sbar Rafiq обновляет себя из приложения (без запроса в терминале), сохранять локальные правки исходников (stash) или выбрасывать (discard). Обновления из терминала всегда спрашивают.'
      }
    }),
    about: {
      ...
      bundleOutOfSyncDesc:
        'Рантайм Sbar Rafiq обновлён, но само приложение — ещё старая сборка: новые функции интерфейса (например, Bot Mode) не появятся до обновления. Запустите обновление ниже, чтобы пересобрать приложение. Если предупреждение не исчезнет, переустановите с последнего установщика.',
      ...
      automaticUpdatesDesc: 'Sbar Rafiq автоматически проверяет обновления в фоне и сообщает, когда они готовы.',
    ...
    config: {
      ...
      loading: 'Загрузка конфигурации Sbar Rafiq…',
    ...
    screenshot: {
      ...
      enabledDesc:
        'Нажмите обе клавиши Command одновременно в любом приложении, чтобы снять его переднее окно и прикрепить снимок к текущему черновику Sbar Rafiq. Автоматической отправки нет. По умолчанию выключено; действует только на этом Mac. Окно может содержать конфиденциальные данные — проверьте вложение перед отправкой.',
      ...
      inputPermission:
        'Разрешение на мониторинг ввода позволяет Sbar Rafiq распознавать обе клавиши Command, когда активно другое приложение. Разрешите Sbar Rafiq доступ в Системных настройках → Конфиденциальность и безопасность → Мониторинг ввода, затем вернитесь сюда и повторите попытку.',
      screenPermission:
        'Разрешение на запись экрана позволяет Sbar Rafiq снимать переднее окно приложения при использовании этого сочетания. Разрешите Sbar Rafiq доступ в Системных настройках → Конфиденциальность и безопасность → Запись экрана, затем вернитесь сюда и повторите попытку. Перезапустите Sbar Rafiq, если macOS попросит.',
    ...
    quickEntry: {
      ...
      enabledDesc:
        'Глобальным горячим ключом вызывайте маленький композер откуда угодно и отправляйте запрос, не открывая Sbar Rafiq.',
    ...
    connections: {
      ...
      intro:
        'Управляйте этим устройством и всеми шлюзами Sbar Rafiq, до которых можно дотянуться через удалённые, SSH или Cloud-соединения.',
      stagedNote:
      ...
      updateSkippedCloud: 'Управляется Sbar Rafiq Cloud',
      kindLocal: 'Локальный',
      kindRemote: 'Удалённый шлюз',
      kindCloud: 'Sbar Rafiq Cloud',
      kindSsh: 'SSH',
      kindLocalDesc: 'Рантайм Sbar Rafiq, которым управляет это приложение.',
      kindRemoteDesc: 'Шлюз Sbar Rafiq, доступный по HTTP(S) — LAN, Tailscale или интернет.',
      kindCloudDesc: 'Хостинговая инстанция, обнаруженная через ваш аккаунт Sbar Rafiq Cloud.',
      kindSshDesc: 'Установка Sbar Rafiq, доступная по SSH.',
      labelTitle: 'Имя',
      labelDesc:
      ...
      headersDesc:
        'Отправляются с каждым HTTP- и WebSocket-запросом к этому шлюзу — например, для прокси доступа вроде Cloudflare Access (CF-Access-Client-Id / CF-Access-Client-Secret). Значения хранятся зашифрованными. Заголовки, которыми управляет Sbar Rafiq (Authorization, Cookie, Host…), игнорируются.',
      ...
      cloudAddHint:
        'Подсказка: вход в Sbar Rafiq Cloud выше автоматически обнаруживает ваших агентов — эту форму используйте только для ручной регистрации известного URL инстанса.',
    ...
    gateway: {
      ...
      intro:
        'По умолчанию — локальный. Используйте удалённый, когда приложение должно управлять бэкендом Sbar Rafiq в другом месте. Соединения шлюзов — на уровне машины; профили обнаруживаются из подключённых шлюзов.',
      ...
      localDesc: 'Запускает приватный бэкенд Sbar Rafiq на localhost. Это значение по умолчанию, работает офлайн.',
      remoteTitle: 'Удалённый шлюз',
      remoteDesc: 'Подключает это приложение к удалённому бэкенду Sbar Rafiq.',
      remoteAuthHint:
      ...
      cloudDesc: 'Войдите в Sbar Rafiq Cloud один раз и выбирайте агентов из своего аккаунта — без вставки URL.',
      cloudSignInTitle: 'Sbar Rafiq Cloud',
      cloudSignIn: 'Войти в Sbar Rafiq Cloud',
      cloudSignedIn: 'Вы вошли в Sbar Rafiq Cloud',
      cloudNeedsSignIn: 'Войдите в Sbar Rafiq Cloud, чтобы обнаружить агентов в вашем аккаунте.',
      ...
      cloudDiscoverFailed: 'Не удалось загрузить агентов Sbar Rafiq Cloud',
      cloudConnectFailed: 'Не удалось подключиться к этому агенту',
      cloudSignInFailed: 'Не удалось войти в Sbar Rafiq Cloud',
      cloudSignedOutTitle: 'Выход из Sbar Rafiq Cloud',
      cloudSignedOutMessage: 'Сессия Sbar Rafiq Cloud сброшена.',
      ...
      restartingMessage: 'Sbar Rafiq Desktop переподключится с сохранёнными настройками — оболочка останется открытой.',
      savedMessage: 'Сохранено для следующего перезапуска.',
      connectedTo: (baseUrl, version) => `Подключено к ${baseUrl}${version ? ` · Sbar Rafiq ${version}` : ''}`,
      ...
      sshDesc:
        'Sbar Rafiq запускается на удалённой машине по SSH и туннелируется в это приложение — ничего не нужно запускать или открывать самим. Требуется рабочая SSH-аутентификация по ключу на хост.',
      sshTrustHint:
      ...
      sshHermesPathTitle: 'Путь к Sbar Rafiq (необязательно)',
      ...
      sshReachable: (host, platform) => `Доступен: ${host} (${platform}) — Sbar Rafiq найден`,
      ...
      sshErrAuth:
        'SSH-аутентификация не удалась. Загрузите ключ в ssh-agent (ssh-add) или задайте IdentityFile в ~/.ssh/config — Sbar Rafiq запускает ssh в неинтерактивном режиме.',
      sshErrHostKey:
      ...
      sshErrNotInstalled:
        'Sbar Rafiq не установлен на удалённой машине. Установите его там (curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh) или задайте путь к Sbar Rafiq.',
      sshErrPlatform:
        'Неподдерживаемая удалённая платформа. SSH-режим Sbar Rafiq Desktop поддерживает удалённые хосты Linux, macOS и Windows.',
      sshErrTimeout: 'SSH-соединение истекло. Хост может быть недоступен или «спит».',
      sshErrUpdateRequired: 'Перед подключением через SSH обновите Sbar Rafiq на удалённой машине.',
    ...
    mcp: {
      ...
      deepLinkDescription:
        'Ссылка запросила добавить этот MCP-сервер в Sbar Rafiq. Проверьте конфигурацию ниже — она пришла из ссылки, а не из Sbar Rafiq.',
    ...
    providers: {
      ...
      intro:
        'Войдите по подписке — копировать API-ключ не нужно. Sbar Rafiq проведёт вход в браузере прямо здесь, в приложении.',
      ...
      localEndpoint: {
        ...
        description: 'Направьте Sbar Rafiq на любой OpenAI-совместимый эндпоинт (Zyphra, vLLM, llama.cpp, Ollama и т. д.).'
      },
  ...
  starmap: {
    ...
    emptyDesc: 'По мере того как Sbar Rafiq создаёт навыки и память для вашей работы, они появятся здесь.',
  ...
  threads: {
    close: 'Закрыть потоки',
    title: 'Потоки',
    subtitle: 'Работа, продолжившаяся после окончания хода.',
    emptyTitle: 'Потоков пока нет',
    emptyDesc: 'Когда этот разговор делегирует работу в фоне, каждый поток появится здесь со своей записью.',
    working: 'В работе',
    workingDesc: 'Выполняется сейчас.',
    resolved: 'Завершено',
    resolvedDesc: 'Завершённые потоки хранятся здесь.',
    failed: 'Сбой',
    failedDesc: 'Остановлен до завершения.',
    waiting: 'Ждёт вас',
    waitingDesc: 'Решения, проверки и разрешения, которые можете дать только вы.',
    live: 'В эфире',
    subagents: 'Субагенты',
    messages: 'Сообщения',
    ...
    model: 'Модель',
    started: 'Начало',
    openThread: 'Открыть поток',
    backToThreads: 'Назад к потокам',
    transcript: 'Запись',
    transcriptTail: 'Показаны последние сообщения.',
    transcriptEmpty: 'Этот поток пока ничего не записал.',
    transcriptFailed: 'Не удалось прочитать этот поток.',
    steerPlaceholder: 'Направить этот поток...',
    steerSend: 'Отправить в поток',
    steerSent: 'Отправлено в поток.',
    steerFailed: 'Не удалось связаться с этим потоком.',
    steerUnavailable: 'Этот поток завершён, направлять нечего.',
    routedTo: 'Отправлено в поток',
    routeUndo: 'Отменить',
    refresh: 'Обновить потоки',
    loading: 'Чтение потоков...'
  },
  agents: {
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'Перезапустите Sbar Rafiq, чтобы использовать питомцев — бэкенд старше этой функции.',
    ...
    generatePet: {
      ...
      staleBackend: 'Обновите Sbar Rafiq, чтобы генерировать питомцев.',
      backgroundHint: 'Можно закрыть — Sbar Rafiq уведомит, когда будет готово.',
    ...
    hermesActiveSessions: (version, count) => `Sbar Rafiq ${version} · Активные сеансы: ${count}`,
  ...
  messaging: {
    ...
    telegramQr: {
      ...
      subtitle:
        'Оба способа подключают бота под вашим контролем и сохраняют его данные только в этой установке Sbar Rafiq.',
      quickSetup: 'Быстрая настройка',
      recommended: 'Рекомендуется',
      quickHelp:
        'Отсканируйте QR-код и подтвердите в Telegram. Sbar Rafiq создаст бота и определит ваш Telegram ID автоматически.',
  ...
  profiles: {
    ...
    defaultDescription: 'Используется при запуске Sbar Rafiq и для новых чатов. Профили существующих сессий не меняются.',
    ...
    deleting: 'Удаление...',
    createDesc: 'Профили — это независимые среды Sbar Rafiq: отдельные настройки, навыки и SOUL.md.',
    nameLabel: 'Имя',
  ...
  cron: {
    ...
    newCron: 'Новый cron',
    emptyDescNew:
      'Запланируйте промпт, который будет выполняться по cron-выражению. Sbar Rafiq выполнит его и доставит результаты в выбранное вами место.',
  ...
  projectWorkflows: {
    ...
    hint: 'Подготовьте черновик новой задачи, проверьте и отправьте его из поля ввода.',
    workflow: 'Тип работы',
    approach: 'Подход к задаче',
    approachHint: 'Определяет объём работы и проверок. Не меняет модель и не задаёт таймер остановки.',
    task: 'Опишите нужный результат',
    preparing: 'Подготовка черновика…',
    openDraft: 'Открыть черновик задачи',
    names: {
      'quick-ui': 'Быстрое исправление интерфейса',
      'client-delivery': 'Передача клиенту',
      research: 'Исследование проекта',
      'weekly-review': 'Обзор недели'
    },
    descriptions: {
      'quick-ui': 'Исправить нужный интерфейс и проверить его в браузере.',
      'client-delivery': 'Подготовить и проверить файлы перед передачей.',
      research: 'Исследовать вопрос с источниками и явными пробелами.',
      'weekly-review': 'Подвести итоги проекта и предложить приоритеты на следующую неделю.'
    },
    approaches: {
      quick: 'Быстрый',
      standard: 'Обычный',
  ...
  projectActions: {
    title: 'Действия из документов',
    hint: 'Проверьте предложения с цитатами перед сохранением задач. Принятые задачи ожидают ручного планирования на общей доске.',
    source: 'Выберите индексированный документ',
    extract: 'Подготовить задачу извлечения',
    refresh: 'Обновить предложения',
    empty: 'Предложений пока нет',
    owner: 'Ответственный, если известен',
    due: 'Срок как в источнике, если известен',
    save: 'Сохранить изменения',
    accept: 'Сохранить как задачу',
    dismiss: 'Отклонить',
    pending: 'Ожидает проверки',
    accepting: 'Повторить сохранение задачи',
    accepted: 'Задача сохранена',
    dismissed: 'Отклонено',
    task: 'Задача',
    board: 'Общая доска',
    edited: 'Поля изменены вами',
    quote: 'Цитата из источника'
  },
  projectBrief: {
    retry: 'Повторить',
    openSource: 'Открыть текущий исходный файл',
    title: 'Сводка проекта и источники',
    hint: 'Поиск по документам с сохранёнными цитатами. Обновите индекс после изменения файлов.',
    summary: 'Сводка проекта',
    save: 'Сохранить сводку',
    sources: 'Папки источников',
    unavailable: 'Недоступно',
    refresh: 'Обновить источники',
    scanLimit: 'Достигнут предел количества файлов. Уточните папки источников.',
    approved: 'Утверждённые результаты',
    search: 'Поиск по арабским или английским источникам',
    history: 'Включить предыдущие версии',
    noMatches: 'Совпадений в индексе нет. Уточните ключевые слова или обновите источники.',
    page: 'Страница',
    paragraph: 'Абзац',
    comment: 'Комментарий',
    line: 'Строки',
    current: 'Текущая версия в индексе',
    previous: 'Предыдущая версия или недоступный источник',
    copy: 'Копировать цитату',
    indexed: (ready, total) => `В индексе ${ready} из ${total} файлов`,
    pending: count => `Ожидают индексации: ${count}`,
    issues: count => `Требуют внимания: ${count}`
  },
  artifacts: {
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'Обновите бэкенд Sbar Rafiq, чтобы создавать проекты — ваш бэкенд старше этого desktop-приложения (Настройки → Обновления → Бэкенд).',
      deleteConfirm: 'Это удалит сохранённый проект из Sbar Rafiq. Файлы, git-репозитории и worktrees не пострадают.',
      ...
      worktreeStaleBackend:
        'Обновите бэкенд Sbar Rafiq, чтобы создавать worktrees по этому удалённому соединению — он старше git worktree API.',
    ...
    row: {
      ...
      chipWorking: 'Работает',
      chipNeedsInput: 'Ждёт вас',
      chipDone: 'Готово',
      chipBackground: 'Фон',
  ...
  composer: {
    ...
    placeholderStarting: 'Запуск Sbar Rafiq...',
    placeholderReconnecting: 'Переподключение к Sbar Rafiq…',
    placeholderFollowUp: 'Отправить продолжение',
    newSessionPlaceholders: [
      'Что будем делать?',
      'Дайте Sbar Rafiq задачу',
      'О чём вы думаете?',
      'Опишите, что нужно',
      'Что обсудим?',
    ...
    attachUrlDesc: 'Sbar Rafiq загрузит страницу и добавит её как контекст для этого хода.',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'Попросить Sbar Rafiq открыть PR',
  ...
  updates: {
    stages: {
      ...
      pull: 'Почти готово…',
      pydeps: 'Завершаем…',
      update: 'Обновляем Sbar Rafiq…',
      rebuild: 'Пересобираем desktop-приложение…',
      restart: 'Перезапускаем Sbar Rafiq…',
    ...
    availableBody: 'Новая версия Sbar Rafiq готова к установке.',
    availableTitleBackend: 'Доступно обновление бэкенда',
    availableBodyBackend: 'Новая версия подключённого бэкенда Sbar Rafiq готова к установке.',
    ...
    manualBody:
      'Sbar Rafiq установлен из командной строки, поэтому обновления тоже выполняются там. Вставьте это в терминал:',
    manualPickedUp: 'Sbar Rafiq подхватит новую версию при следующем запуске.',
    guiSkewTitle: 'Обновите desktop-приложение',
    guiSkewBody:
      'Бэкенд обновлён, но пакет этого desktop-приложения не изменился. Обновите или переустановите desktop-приложение Sbar Rafiq (ваш AppImage / .deb / .rpm), чтобы версии совпали.',
    copy: 'Копировать',
    copied: 'Скопировано',
    done: 'Готово',
    applyingBody:
      'Обновлятор Sbar Rafiq возьмёт процесс на себя в своём окне и автоматически перезапустит Sbar Rafiq, когда закончит. Пожалуйста, не открывайте Sbar Rafiq вручную во время обновления.',
    applyingBodyBackend:
      'Удалённый бэкенд применяет обновление и перезапустится. Sbar Rafiq переподключится автоматически, когда он снова заработает.',
    applyingClose: 'Это окно закроется во время обновления, затем Sbar Rafiq откроется сам.',
    errorTitle: 'Обновление не завершилось',
    errorBody: 'Не переживайте — ничего не потеряно. Можно попробовать снова прямо сейчас.',
    blockerTitle: 'Закрыть локальные предпросмотры, чтобы обновить Sbar Rafiq?',
    blockerBody:
      'Sbar Rafiq нужно остановить эти локальные предпросмотры перед обновлением. Ваши файлы не будут изменены или удалены.',
    foreignBlockerTitle: 'Закрыть другие процессы, чтобы обновить Sbar Rafiq',
    foreignBlockerBody:
      'Sbar Rafiq не может безопасно закрыть эти процессы автоматически. Закройте приложение, терминал или службу, которой принадлежит каждый из них, и повторите обновление.',
    mixedBlockerBody:
      'Sbar Rafiq может закрыть локальные предпросмотры, перечисленные ниже. Остальные процессы нужно закрыть вручную, прежде чем обновление сможет продолжиться.',
  ...
  install: {
    ...
    oneTimeTitle: 'Sbar Rafiq требует одноразовой установки',
    ...
    setupChoiceTitle: 'Настройка Sbar Rafiq Desktop',
    setupChoiceDesc:
      'Подключите это приложение к уже работающему шлюзу Sbar Rafiq или установите Sbar Rafiq локально на этот компьютер.',
    connectExistingTitle: 'Подключиться к существующему Sbar Rafiq',
    ...
    installLocalTitle: 'Установить Sbar Rafiq локально',
    installLocalDesc: 'Скачайте Sbar Rafiq, создайте его Python-окружение и запустите бэкенд на этом компьютере.',
    localStartUnavailable: 'Не удалось начать локальную установку. Перезапустите Sbar Rafiq Desktop и попробуйте снова.',
    remoteSetupTitle: 'Подключиться к существующему Sbar Rafiq',
    remoteSetupDesc: 'Введите URL вашего шлюза. Sbar Rafiq Desktop определит, нужен токен или вход через браузер.',
    remoteUrlTitle: 'URL шлюза',
    remoteUrlDesc: 'Используйте базовый URL шлюза Sbar Rafiq, включая https:// для удалённых.',
    remoteUrlPlaceholder: 'https://gateway.example.com/hermes',
    probing: 'Определяем аутентификацию шлюза...',
    probeError: 'Не удалось подключиться к этому шлюзу Sbar Rafiq.',
    ...
    settingUpTitle: 'Настройка Sbar Rafiq Agent',
    finishingTitle: 'Завершаем',
    failedDesc:
      'Один из шагов установки завершился ошибкой. На Windows это может произойти, если запущена другая инстанция Sbar Rafiq CLI или desktop. Остановите все работающие инстанции Sbar Rafiq и повторите. Подробности — ниже или в журнале desktop.',
    activeDesc:
      'Это одноразовая настройка. Установщик Sbar Rafiq скачивает зависимости и настраивает вашу машину. При следующих запусках этот шаг будет пропущен.',
  ...
  onboarding: {
    ...
    preparingInstall: 'Sbar Rafiq завершает установку. Обычно это занимает меньше минуты при первом запуске.',
    starting: 'Запускаем Sbar Rafiq…',
    ...
    recommended: 'Рекомендуется',
    connected: 'Подключено',
    featuredPitch: 'Одна подписка, 300+ передовых моделей — рекомендуемый способ запускать Sbar Rafiq',
    ...
    flowSubtitles: {
      ...
      device_code: 'Откроет страницу подтверждения в браузере — Sbar Rafiq подключится автоматически',
    ...
    authorizeThere: 'Авторизуйте Sbar Rafiq там.',
    ...
    autoBrowser: provider =>
      `Мы открыли ${provider} в вашем браузере. Авторизуйте Sbar Rafiq там, и подключение произойдёт автоматически — ничего копировать и вставлять не нужно.`,
  ...
  preview: {
    ...
    office: {
      converting: 'Печать точных страниц через LibreOffice',
      downloaded: 'Загружено в это рабочее пространство',
      document: 'ДОКУМЕНТ',
      emptyBook: 'В этой книге нет листов.',
      emptyDeck: 'В этой презентации нет слайдов.',
      ...
      sheetTruncated: 'Эта книга очень большая. Показаны первые строки и столбцы каждого листа.',
      slideNumber: index => `Слайд ${index}`,
      zoomIn: 'Увеличить',
      zoomOut: 'Уменьшить'
    },
    ...
    largeBody: (label, size) => `${label} — ${size}. Sbar Rafiq покажет только первые 512 КБ.`,
    ...
    web: {
      ...
      tryAgain: 'Попробовать снова',
      restarting: 'Sbar Rafiq перезапускается...',
      askRestart: 'Попросить Sbar Rafiq перезапустить сервер',
      lookingRestart: taskId => `Sbar Rafiq ищет сервер предпросмотра для перезапуска (${taskId})`,
      restartingTitle: 'Перезапуск сервера предпросмотра',
      restartingMessage: 'Sbar Rafiq работает в фоне. Следите за прогрессом в консоли предпросмотра.',
      ...
      blankPageBody: 'Введите адрес выше, чтобы просматривать, или попросите Sbar Rafiq открыть страницу.',
      finishedRestarting: message => `Sbar Rafiq завершил перезапуск сервера предпросмотра${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'Sbar Rafiq не смог перезапустить сервер.',
      stillWorking:
        'Sbar Rafiq всё ещё работает, но результата перезапуска пока нет. Команда сервера может выполняться в foreground.',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'Sbar Rafiq загружает ответ',
      ...
      turnDigestNotes: count => `${count} ${count === 1 ? 'заметка' : 'заметок'}`,
      turnOutcomeDelivered: 'Сделано',
      turnOutcomeFailed: 'Не удалось',
      turnOutcomeOpen: 'Осталось',
      turnOutcomeTitle: 'Итог хода',
  ...
  desktop: {
    ...
    restartToUseSaveImage: 'Перезапустите Sbar Rafiq Desktop, чтобы использовать «Сохранить изображение».',
    restartToSaveImages: 'Перезапустите Sbar Rafiq Desktop, чтобы сохранять изображения',
