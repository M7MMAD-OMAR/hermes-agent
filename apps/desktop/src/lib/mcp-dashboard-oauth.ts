...
const UPDATE_BACKEND = 'Update the Sbar Rafiq backend to support Desktop MCP OAuth callbacks.'
...
export async function completeMcpDesktopOAuth({
  ...
  if (!bridge && scope.connectionId !== 'local') {
    throw new Error('Update Sbar Rafiq Desktop to support MCP OAuth callbacks.')
  }
