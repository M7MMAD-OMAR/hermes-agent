  ...
  it('fails fast for an unscoped request with no session in view', () => {
    ...
    expect(respond).toHaveBeenCalledWith({
      value: JSON.stringify({
        // This fork names the session: the bare sentence sent the agent hunting
        // for a window to focus when the real answer is which chat asked.
        error:
          'The in-app browser only takes actions for a session that is open on screen. ' +
          'Session (none) has no open surface in this window.',
        success: false
      })
    })
  })
