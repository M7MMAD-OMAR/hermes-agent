 ...
 *  clients (Sbar Rafiq Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Sbar Rafiq"
 *  (community report, Aug 21 2026: renamed default still read "Sbar Rafiq is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Sbar Rafiq (matching displayName and the
 ...
 *  failing `default`s are never one anonymous "Sbar Rafiq" — judged against the
...
export function groupSpeakerLabel(name?: null | string, group?: null | string) {
  ...
  if (boundary !== -1) {
    ...
    const label = title || (profile.toLowerCase() === 'default' ? 'Sbar Rafiq' : profile)
  ...
  // title, then the local row's display_name, then default → Sbar Rafiq.
  ...
  return isDefault ? 'Sbar Rafiq' : trimmed
}
