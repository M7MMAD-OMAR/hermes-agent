    ...
    () =>
      onComposerSubmitRequest(({ attachments, surfaceId: requestedSurfaceId, target, text, displayKind }) => {
        if (
          ...
          if (!current.busy) {
            current.dispatchSubmit(text, attachments, displayKind)
          ...
          // The unsent draft and its own chips stay in the composer; only what
          // the request carried is queued with it.
          const enqueue = () =>
