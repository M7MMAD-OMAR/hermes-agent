...
interface Secondary {
  ...
  /** When the current failure streak began (ms epoch); only meaningful while
   *  `livenessProbeFailures` is above zero. The policy's deferral budget is
   *  measured from here. */
  livenessFirstFailureAt: number
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Sbar Rafiq gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.hermesDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Sbar Rafiq Desktop.')
  }
...
export async function ensureActiveGatewayOpen({
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Sbar Rafiq gateway is not connected" on the Sessions `+` action (#88880).
...
function probeSecondaryLiveness(entry: Secondary): void {
  void entry.gateway.request('ping', {}, LIVENESS_PROBE_TIMEOUT_MS).then(
    ...
    (error: unknown) => {
      ...
      if (entry.livenessProbeFailures === 0) {
        entry.livenessFirstFailureAt = Date.now()
      }
      ...
      const decision = decideLivenessForceClose({
        ...
        deferredForMs: Date.now() - entry.livenessFirstFailureAt
      })
      ...
      if (!decision.close) {
        if (entry.livenessReprobeTimer === null) {
          ...
          }, livenessReprobeDelayMs(entry.livenessProbeFailures))
        }
