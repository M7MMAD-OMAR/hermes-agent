...
| working branch | `sbar-rafiq` — **not** `main` |

`main` is a stale local ref. All work is on `sbar-rafiq`, which
...
```bash
git worktree add -b merge/upstream-<date> /tmp/merge-wt sbar-rafiq
