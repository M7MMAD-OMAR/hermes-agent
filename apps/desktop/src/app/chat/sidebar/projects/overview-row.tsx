...
export function ProjectOverviewRow({
  ...
  const showAll = () => {
    ...
    setExpanding(true)
    // No `supersedable` flag: this fork joins the in-flight read per
    // (profile, project) instead of superseding, so a second expansion can
    // never leave the first row collapsed. A failed read still falls back to
    // entering the project, which the tagged result now says explicitly rather
    // than through a rejection.
    fetchProjectSessions(project.id)
      .then(result => {
        if (result.status === 'ok' && result.project) {
          setExpanded(result.project)
