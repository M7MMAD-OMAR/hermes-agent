...
function SidebarRail({ className, ...props }: React.ComponentProps<'button'>) {
  ...
  // The rail's geometry is one coordinate system keyed off the `side` prop and
  // the matching `data-side` attribute, so its left/right offsets stay physical.
  // Converting only some of them would make the hairline change sides between
  // the collapsed and expanded states under RTL.
  return (
    <Tip label={label}>
      <button
        aria-label={label}
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      className={cn(
        'flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium text-sidebar-foreground/70 ring-sidebar-ring outline-hidden transition-[margin,opacity] duration-200 ease-linear focus-visible:bg-sidebar-accent focus-visible:text-sidebar-accent-foreground focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
...
function SidebarMenuButton({
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent hidden={state !== 'collapsed' || isMobile} placement="left-rail" {...tooltip} />
    </Tooltip>
  )
}
