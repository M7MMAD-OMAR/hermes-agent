...
def _trunk_placement(repo_root: str, branch: str) -> dict:
    # An unrecorded branch folds into the trunk lane so a repo never shows two
    # trunk lanes. The fallback must be the repo's ACTUAL branch: a fixed
    # "main" forks a phantom lane on every repo whose trunk is named anything
    # else, and that lane takes almost every row with it.
