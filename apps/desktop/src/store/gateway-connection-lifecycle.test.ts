...
describe('reconnectSecondaryGateways', () => {
  ...
  it('closes a live-in-use secondary on a forced wake only when its liveness probe fails', async () => {
    ...
    // The bounded re-probe also goes unanswered. A turn is still in flight, so
    // the socket keeps its deferral until the policy's time budget is spent,
    // the same budget the primary's probe applies.
    await vi.advanceTimersByTimeAsync(LIVENESS_REPROBE_DELAY_MS)

    expect(socket.close).not.toHaveBeenCalled()

    // Past the budget, the next unanswered re-probe tears a persistently
    // unresponsive backend down so its reconnect backoff can heal it: it is
    // never trusted forever. Re-probes back off up to the cap, so the budget
    // is judged at most one capped interval late.
    await vi.advanceTimersByTimeAsync(LIVENESS_MAX_DEFERRAL_MS + LIVENESS_MAX_REPROBE_DELAY_MS)
