...
# ── local to this fork ────────────────────────────────────────────────────────────────────────
#
# Two events this fork emits that upstream has no emitter for. They are declared here rather than
# patched into the generated TS, because the generator treats Python as the source and a hand-edit
# to `gateway-contract.generated.ts` is overwritten on the next run.


class NextMove(Payload):
    """One suggestion in a `next_moves.offer` pack (``agent/next_moves.py::NextMove.as_dict``)."""
...
class NextMovesOfferPayload(Payload):
    """``agent/next_moves.py::emit_next_moves`` - the post-turn suggestion pack the composer
    offers as ghost text, taken with Tab."""

    moves: list[NextMove] = Field(default_factory=list)
    session_id: str
    source: str
    turn_id: str


event("next_moves.offer", NextMovesOfferPayload,
      doc="Post-turn suggestions for the composer's ghost text. Local to this fork.")


class TurnOutcome(Payload):
    """``agent/turn_outcome.py::Outcome.as_dict`` - what the finished turn settled as."""
...
class SessionOutcomePayload(Payload):
    """``agent/turn_outcome.py::emit_turn_outcome`` - delivered / failed / open for the turn that
    just settled, carrying the CLIENT turn id so it lands on the row it describes."""
...
event("session.outcome", SessionOutcomePayload,
      doc="How the finished turn settled, landed on its final assistant row. Local to this fork.")
