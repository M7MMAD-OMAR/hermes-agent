...
_ACTIONS = {
    # `new_tab` is local and must survive the consolidation into one
    # `desktop_preview` tool: a second tab is how one conversation holds two
    # pages side by side, which is the point of the agent's own browser.
    "open": lambda args: open_preview_tool(
        url=args.get("url", ""),
