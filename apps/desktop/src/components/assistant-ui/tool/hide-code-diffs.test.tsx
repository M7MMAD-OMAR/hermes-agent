...
it('keeps edit counts without code in either display mode and restores disclosure when disabled', async () => {
  for (const toolName of ['patch', 'edit_file', 'write_file']) {
    ...
    // Rows start collapsed in this build; open the edit the way a reader would.
    if (!row.hasAttribute('data-tool-open')) {
      fireEvent.click(toggle)
    }

    await waitFor(() => expect(row.hasAttribute('data-file-edit')).toBe(true))
