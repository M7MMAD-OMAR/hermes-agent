...
function AttachmentPill({ attachment, onRemove }: { attachment: ComposerAttachment; onRemove?: (id: string) => void }) {
  ...
  async function openAttachment() {
    ...
    if (attachment.kind === 'image') {
      try {
        const source = lightboxSrc || (await attachmentImageDataUrl([attachment.path, attachment.detail]))
    ...
    try {
      if (!(await openAttachmentPreview(rawTarget, cwd || undefined))) {
        throw new Error(c.couldNotPreview(attachment.label))
      }
    } catch (error) {
      notifyError(error, c.previewUnavailable)
        ...
        <div className="group/attachment relative min-w-0 shrink-0">
          ...
          {onRemove && (
            <button
              ...
              className="absolute -end-1 -top-1 grid size-3.5 place-items-center rounded-full border border-border/70 bg-background text-muted-foreground opacity-0 shadow-xs transition hover:bg-accent hover:text-foreground group-hover/attachment:opacity-100 focus-visible:opacity-100"
