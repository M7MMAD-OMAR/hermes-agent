...
def _scoped_sid(session_id: str) -> str:
    ...
    Every cache path, lookup, install and release, goes through this, so release finds what lookup
    made. The surface half is added on top by ``_cache_key``."""
...
def _get_backend(session_id: str = "", surface: str = "") -> ComputerUseBackend:
    # Approval state is keyed by the raw Hermes session id, the cache by the
    # profile-scoped one. `_backend_for_call` re-checks the mode with the raw id,
    # so resolving it here from the scoped id would never match under a profile
    # override and that loop would spin.
    bare_sid = str(session_id or "")
    sid = _cache_key(_scoped_sid(session_id), surface)
    while True:
        with _backend_lock:
...
def _backend_for_call(session_id: str = "", surface: str = "") -> Iterator[ComputerUseBackend]:
    ...
    # Keyed per surface like ``_get_backend``, so a device call and a desktop call in one
    # session hold independent locks and never queue behind each other.
    sid = _cache_key(_scoped_sid(session_id), surface)
    while True:
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    # The lease fences the profile's desktop. A phone is its own screen, so a human
    # holding the desktop does not stop the agent driving a device, and a device
    # call has no reason to start the Bot Desktop.
    on_desktop = (surface or _configured_surface() or "desktop") in _DESKTOP_SURFACES
    admitted = None
    if on_desktop:
        try:
    ...
    try:
        backend = call.enter_context(_backend_for_call(session_id, surface))
    except Exception as e:
    ...
    try:
        with call:
            ...
            def _fence() -> None:
                ...
                if admitted is not None and _bd_lease.get().epoch != admitted.epoch:
