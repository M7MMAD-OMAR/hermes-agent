...
import { $graftSavingsBySession, graftSavingsFor, recordGraftToolResult, resetGraftSavings } from './graft-savings'
...
describe('graft-savings store', () => {
  it('accumulates tokens and calls per session', () => {
    ...
    expect(graftSavingsFor('s1')).toEqual({ calls: 2, tokensSaved: 17954 })
    expect(graftSavingsFor('s2')).toEqual({ calls: 1, tokensSaved: 17234 })
    expect(graftSavingsFor('s3')).toBeNull()
    expect(graftSavingsFor(null)).toBeNull()
  })
  ...
  it('counts a call that reported no savings line as a call with zero saved', () => {
    ...
    expect(graftSavingsFor('s1')).toEqual({ calls: 1, tokensSaved: 0 })
  })
  ...
  it('counts each tool id once even when tool.complete is replayed', () => {
    ...
    expect(graftSavingsFor('s1')).toEqual({ calls: 1, tokensSaved: 17234 })
  })
  ...
  it('reads MCP content-block results too', () => {
    ...
    expect(graftSavingsFor('s1')?.tokensSaved).toBe(17234)
  })
})
