...
PLATFORM_HINTS = {
    ...
    "desktop": (
        ...
        # The fence-routing sentence is load-bearing, not decoration:
        # tests/agent/test_desktop_capabilities_are_advertised.py reads the
        # renderer and fails when a fence it handles goes unadvertised here.
        "You are chatting inside the Hermes desktop app, a graphical chat surface. Markdown renders with full GitHub "
        "flavor (tables, syntax-highlighted code, math via $...$, task lists, callouts). "
        "Fences are routed, not all plain code: ```mermaid ALWAYS draws in place (theme-aware, zoomable, copyable "
        "as PNG) — reach for it whenever a flow, architecture, sequence, state machine, timeline or relationship "
        "lands better drawn than described; a small ```svg renders as sanitised inline SVG; a ```listing fence "
        "carrying JSON (an array, or {listings:[...]}) of {address, price, beds, baths, size, note, facts[], "
        "catches[], images[], links[{label,url}]} draws browsable property cards for portals that cannot be framed; "
        "a large graphic, a "
        "```html document, and any fence past ~48 lines promote to an artifact card in the right rail, so a long "
        "file never drowns the chat and you should not truncate it for fear that it will. "
        "Deliver files by writing "
...
# 6: entries also carry upstream's `requires_apps` (upstream's own bump to 3
# collided with the fork's numbering), so the snapshot path can re-check app
# presence.
_SKILLS_SNAPSHOT_VERSION = 6
    ...
    for skill_file in skill_files:
        try:
            ...
            if not entry or fm_name in claimed or hides(fm_name, entry["skill_name"], extract_skill_conditions(frontmatter)):
                continue
            claimed.add(fm_name)
            indexed = _index_description(entry["description"], entry.get("tags"))
            skills_by_category.setdefault(entry["category"], []).append((fm_name, f"{desc_prefix}{indexed}".strip()))
        except Exception as e:
...
def _label_visible_entries(visible_entries: list[dict], skills_by_category: dict[str, list[tuple[str, str]]]) -> None:
    ...
    for entry in visible_entries:
        fm, org_id = _entry_name(entry), entry.get("org_id")
...
def _build_skills_system_prompt_inner(
    ...
    if snapshot is not None:
        # skill_matches_environment is evaluated at PARSE time on the cold path
        # (_parse_skill_file) and therefore NOT here, where entries come off
        # disk. Re-apply it: without this, an entry written while kanban was
        # active outlives the environment that justified it, so the index gains
        # a line on snapshot-backed builds and loses it on cold ones, which
        # rewrites the cached system prefix instead of hitting it. Snapshot
        # v5 is what carries `environments` for this.
        ...
        candidates = [(entry, skill_matches_platform_list(entry.get("platforms") or [])
                       and skill_matches_environment({"environments": entry.get("environments")})
