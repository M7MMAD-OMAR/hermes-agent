...
// Task text is user- or model-written, so it picks its own direction from its
// first strong character instead of inheriting the (possibly Arabic) chrome.
function TaskMarkdown({ text }: { text: string }) {
  return (
    <div
      className="min-w-0 [&_.aui-md>:first-child]:mt-0 [&_.aui-md>:last-child]:mb-0"
      ...
      <div className="relative">
        ...
        <Tip label={sendLabel}>
          <Button
            ...
            className="absolute top-1 end-1 text-muted-foreground/80 hover:text-foreground"
            disabled={empty}
            onClick={submit}
            size="icon-xs"
            type="button"
    ...
    <Dialog onOpenChange={open => !open && onClose()} open>
      ...
      >
        <header className="flex flex-col gap-2 px-5 pt-4 pb-3">
          ...
          <DialogTitle className="leading-snug" data-selectable-text="true" dir="auto">
