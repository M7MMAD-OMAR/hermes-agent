...
def _read_desktop_stamp() -> dict:
    """The desktop build stamp as a dict, or {} when absent/unreadable."""
    try:
...
def _desktop_content_unchanged_via_git(project_root: Path) -> bool:
    """True only when git can cheaply PROVE the hashed paths are unchanged.

    Requires the stamp to carry the HEAD commit its hash was computed against
    (older stamps predate that field and always miss here, which is correct:
    they fall through to the real hash, exactly as before this path existed).
    """
    saved_commit = _read_desktop_stamp().get("gitCommit")
    if not saved_commit:
        return False
    try:
        common = {"cwd": project_root, "capture_output": True, "text": True, "timeout": 5}
        head = subprocess.run(["git", "rev-parse", "HEAD"], **common)
        if head.returncode != 0 or head.stdout.strip() != saved_commit:
            return False
        # git diff catches tracked modifications; git status also catches new
        # untracked files under these paths — the content-hash walk sees those
        # too (it reads the filesystem, not the index), so both are required
        # to match its verdict.
        diff = subprocess.run(["git", "diff", "--quiet", "HEAD", "--", *_DESKTOP_HASH_PATHS], **common)
        if diff.returncode != 0:
            return False
        status = subprocess.run(["git", "status", "--porcelain", "--", *_DESKTOP_HASH_PATHS], **common)
        return status.returncode == 0 and not status.stdout.strip()
    except Exception:
        return False


def _desktop_head_commit(project_root: Path) -> str:
    """HEAD's SHA for the build stamp, or "" off a git checkout.

    Best-effort: a packaged install has no .git, and
    _desktop_content_unchanged_via_git treats a missing gitCommit as "cannot
    fast-path" and always falls through to the real hash.
    """
    try:
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    # Fast path: any uncertainty at all — not a git checkout, git missing, HEAD
    # moved, a scoped change present, any subprocess failure — falls straight
    # through to the full hash below.
    if not source_mode and _desktop_content_unchanged_via_git(project_root):
        return False
...
def _write_desktop_build_stamp(project_root: Path, *, source_mode: bool) -> None:
    ...
    _write_build_stamp(
        ...
        gitCommit=_desktop_head_commit(project_root))
