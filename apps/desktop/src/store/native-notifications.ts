...
import { runtimeHasOpenSurface, storedSessionIdForRuntimeId } from './session-states'
...
// "Backgrounded" = the user isn't on Sbar Rafiq. `document.hidden` only flips when
 ...
 *  user is away from Sbar Rafiq — the in-app toast (`host.notify`) covers the
 *  foreground case. */
