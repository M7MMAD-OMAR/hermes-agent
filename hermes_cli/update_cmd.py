...
def _reconcile_diverged_checkout(git_cmd, branch: str, pre_pull_sha) -> None:
    ...
    # Same branch WITH a common ancestor: the orphan rescue above does not fire,
    # and the reset below drops any local-only commits off every ref. Park them
    # on a real branch first — the reflog is not a backup, it expires.
    _backup_branch = _preserve_local_commits_before_reset(
        git_cmd, _m().PROJECT_ROOT, f"origin/{branch}")
    if _backup_branch:
        print(f"  ⚠ Local-only commits saved on branch '{_backup_branch}' before resetting.")
