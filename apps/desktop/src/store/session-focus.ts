...
export const $focusedTreePaneId = computed(
  ...
  (groupId, tree, workspaceMode, lastContentGroup) => {
    ...
    // An owned workspace's chats (Bots, HerWork) are tiles, never the primary
    // selection. Sidebar roster focus must not publish a null session and let
    // the workspace home reclaim the chat on screen (#96062).
