...
import { I18nProvider, loadTranslations, useI18n } from '@/i18n'
...
// Non-English catalogs load on demand; the sync translator falls back to
// English until each chunk lands, so fetch them all before switching.
beforeAll(async () => {
  await Promise.all(Object.keys(TRANSLATIONS).map(locale => loadTranslations(locale as Locale)))
})
