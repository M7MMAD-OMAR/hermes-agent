...
def _validate_anthropic(req: _Request) -> Optional[dict[str, Any]]:
    ...
    # Disk-cached, not the raw fetcher: this runs on every /model switch.
    models = _m.cached_fetch_anthropic_models(base_url=req.base_url or None, api_key=req.api_key or None)
    if models is None:
        return None
