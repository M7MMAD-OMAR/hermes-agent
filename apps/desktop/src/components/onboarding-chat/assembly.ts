...
import { DEFAULT_TRANSLATIONS, translationsFor } from '@/i18n/catalog'
...
export function pickOnboardingGreeting(): string {
  ...
  // A locale's tree is its own chunk; until it lands this degrades to English,
  // the same fallback every other translate takes (see i18n/catalog.ts).
  const copy = (translationsFor(getRuntimeI18nLocale()) ?? DEFAULT_TRANSLATIONS).guidedGreeting
