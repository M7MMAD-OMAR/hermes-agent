...
# billing/usage/subscription = blocking portal (+Stripe) round-trips; complete.* = git ls-files /
...
# session.context_breakdown = rebuilds the whole system prompt (SOUL.md, context files, memory, the
# skills index) off disk and then walks the transcript twice to size it — tens of ms on every turn end
# and session switch, with prompt.submit / session.interrupt sitting unread behind it. Read-only:
...
def _make_agent(
    ...
    # This gateway is the only surface that dispatches post-turn next moves,
    # so it is the only one that may ask finalize_turn to stage them. Every
...
from . import (  # noqa: E402
    ...
    methods_threads as _methods_threads,
...
for _m in (
    ...
    _methods_session_control, _methods_subagents, _methods_threads, _methods_device, _methods_vault,
    _methods_free_tier, _methods_connectors, _methods_connectors_account, _methods_display,
    _methods_display_watch, _methods_onboarding):
