 ...
 *    desktop shell fires only while the user is AWAY from Sbar Rafiq. This is the
...
function notifyOne(kind: string, spec: { titleKey: string; toast: ToastKind }, ev: CompletionEvent): void {
  ...
  // is away from Sbar Rafiq (the toast above covers the foreground case). Isolated:
