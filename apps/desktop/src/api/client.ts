...
// "Timed out connecting to Sbar Rafiq backend" that hangs the UI (#48504).
//
...
export const GATEWAY_NOT_CONNECTED_MESSAGE = 'Sbar Rafiq gateway is not connected'

export class HermesGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Sbar Rafiq gateway connection closed',
      connectErrorMessage: 'Could not connect to Sbar Rafiq gateway',
      ...
      onUnhandledRequest: request =>
        console.warn(`[gateway] Sbar Rafiq has no server-request registry for ${request.method} (${request.id})`),
