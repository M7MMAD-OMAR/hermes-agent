...
from hermes_state_common import (
    ...
    FTS_STORAGE_VERSION, FTS_TOOL_CONTENT_PREFIX_CHARS, FTS_TRIGRAM_FULL_CONTENT_HIGH_WATER_KEY,
    ...
    def _init_fts(self, cursor: sqlite3.Cursor) -> None:
        ...
        if self._fts_stale:
            ...
            def ensure_trigram() -> bool:
                ...
                if not trigram_fts_config_enabled():
                    self._drop_trigram_fts(cursor)
                    return False
                return self._ensure_fts_schema(cursor, "messages_fts_trigram", trigram_sql)

            def ensure_and_rebuild() -> None:
                self._fts_enabled = self._ensure_fts_schema(cursor, "messages_fts", base_sql)
                if not self._fts_enabled:
                    return
                self._trigram_available = ensure_trigram()
            ...
            if base_triggers_missing:
                ...
                if self._fts_enabled:
                    trigram_enabled = ensure_trigram()
