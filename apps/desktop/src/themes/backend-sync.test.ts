...
describe('ingestBackendSkin', () => {
  ...
  it('seed does not paint, but a later same-name skin.changed applies (missed-activation recovery)', () => {
    ...
    // restarted). Sbar Rafiq re-affirms it — `hermes config set display.skin neon`
    ...
    ingestBackendSkin(skin('forest'), { apply: true }) // Sbar Rafiq authored a new skin
  ...
  it('applies a runtime switch back to default (repaints the desktop to its own default)', () => {
    ...
    ingestBackendSkin(skin('default'), { apply: true }) // Sbar Rafiq switched back to default
