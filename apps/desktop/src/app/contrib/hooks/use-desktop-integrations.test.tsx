...
describe('useDesktopIntegrations', () => {
  ...
  describe('notification click -> focus-session id translation', () => {
    ...
    it('translates a runtime id via the window map before navigating', () => {
      ...
      // What matters here is the id the open was asked for: the stored id the
      // chat route is keyed by, translated from the runtime id the
      // notification carried.
      expect(openSessionMock).toHaveBeenCalledWith('stored-abc', navigate, 'stack', undefined)
    })
    ...
    it('falls back to the durable per-runtime state mirror when the window map has no binding', () => {
      ...
      expect(openSessionMock).toHaveBeenCalledWith('stored-xyz', navigate, 'stack', undefined)
