 ...
 *  the one new row. `scope` is the (connection, profile) shortlist the row
 *  lands in; without it only the slug is remembered. */
export function addCustomModel(provider: string, model: string, row?: ModelOptionProvider, scope?: string): void {
  ...
  const next = resolveVisibleKeys(visibleModelsForScope($visibleModelsByScope.get(), scope), merged)
