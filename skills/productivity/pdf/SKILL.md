...
`pdf_create.py` builds the document with the house design system and reports which one under `"theme"` in its JSON output; the key is `null` when the sibling `house-style` skill is not installed beside this one. The reportlab sample stylesheet (Times-Roman body under Helvetica-Bold headings, at sizes nobody chose) is replaced with the house one, on the same type scale the Word and deck paths use, and the boxed table style is replaced with three horizontal rules and no vertical ones.
...
- **Compression expectations**: `--compress` only deflates content streams. Typical savings are 0 to 20%; it does nothing for PDFs dominated by images or already-compressed streams. It is not a substitute for image downsampling (Ghostscript territory).
...
- Before delivering, lint the file: `python scripts/../../house-style/scripts/style_lint.py out.pdf`. A long dash is an error, not a warning.
