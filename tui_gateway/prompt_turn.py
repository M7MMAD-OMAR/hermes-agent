...
def _dispatch_followup_turn(rid, sid: str, session: dict, prompt: Any, what: str, *,
    ...
    try:
        ...
        _run_prompt_submit(rid, sid, session, prompt, initiator="agent")
...
def _run_prompt_submit(
    ...
    queued_prompt_generation: int | None = None, initiator: str = "user",
    ...
    # Who asked for this turn. Every synthesized source — crash auto-continue,
    # wake-ups, notification drains, /goal continuations, /loop ticks — passes
    ...
    def run():
        ...
        try:
            ...
            # initiator BEFORE the /goal and /loop follow-ups below can
            # resubmit: a continuation sets its own, and this turn's
            # provenance is what decides whether the user drove it.
            try:
