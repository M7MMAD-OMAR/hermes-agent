...
def _import_foreign_resume(args) -> None:
    """--resume @claude / @codex / @kimi: import a foreign session and resume it."""
    _resume_foreign = getattr(args, "resume", None)
    if not (isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in ("@claude", "@codex", "@kimi")):
        return
