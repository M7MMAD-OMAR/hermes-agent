...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Sbar Rafiq doesn't know how to remove it. */
...
// non-desktop) Sbar Rafiq simply carries none of it. See store/profile-share.ts.
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Sbar Rafiq, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with Sbar Rafiq but install on demand. */
 ...
 *  not Sbar Rafiq); elsewhere, driver health from `cua-driver doctor`. `null`
