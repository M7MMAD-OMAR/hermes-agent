...
export function ChatBar({
  ...
  const handlePaste = (event: ClipboardEvent<HTMLDivElement>) => {
    ...
    // A pasted GitHub PR-comment deep link used to be intercepted here: it
    // raised a structured review card and returned, so the clipboard payload
    // never reached the editor and the user saw only a pill. Upstream removed
    // that (paste-url-is-text.test.tsx pins it), and the removal is right: a
    // pasted URL is never swallowed. The review attachment itself stays, raised
    // from the git review pane, and `reviewCommentBlock` still expands it at
    // send time.
    event.preventDefault()
