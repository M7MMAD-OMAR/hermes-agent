...
/**
 * GPT-Live voice chat: the full-duplex voice frontend that DELEGATES to Sbar Rafiq.
 *
 ...
 * that into an ordinary Sbar Rafiq turn on the open session and streams the reply
 * back with `session.commentary.append`, which the voice paraphrases aloud.
 * Sbar Rafiq keeps every capability — model choice, tools, memory, approvals.
 *
...
export interface VoiceLiveHandlers {
  /** GPT-Live asked the backend (Sbar Rafiq) for help. `context` is the recent
...
export class VoiceLiveSession {
  ...
  /** The delegation currently being answered by Sbar Rafiq; late results for an
  ...
  /** Quiet progress for the live model ("Sbar Rafiq is running the tests…"). */
