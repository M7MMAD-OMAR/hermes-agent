...
# the user's main chat model, mapped to whether they ride the fast lane by
# default. The opt-in (or opt-out) lives in
# ``auxiliary.<task>.prefer_fast_model`` so the default ``auto = main model``
# contract remains true on every settings surface. The turn outcome defaults
# on: it is a three-line digest fired after every non-trivial turn, and on the
# main model its cost estimate would be wrong (docs/design/herwork-workspace.md).
_FAST_MODEL_TASKS: Dict[str, bool] = {"next_moves": False, "title_generation": False, "turn_outcome": True}
...
def _try_main_provider_route(
    ...
    # money on OpenRouter — including this Step-1 "use my main model" path.
    # Passing a paid main model through as an explicit override would make
    # _try_openrouter() reject it outright (correct), but that also discards
    # explicit_api_key/base_url above, and Step-2/3's own bare OpenRouter
    # retry has no way to recover them — a user whose OpenRouter access is
    # only the runtime key (no OPENROUTER_API_KEY env var, no credential
    # pool) gets no auxiliary provider at all, on every task, instead of the
    # free SKU they configured via auxiliary.openrouter_model specifically
    # for this. Substitute that configured free model directly — NOT None:
    # resolve_provider_client's own universal pre-fill re-reads the same paid
    # main model straight out of the live runtime context when handed a blank
    ...
    client, resolved = resolve_provider_client(
        resolved_provider, step1_model, explicit_base_url=explicit_base_url,
