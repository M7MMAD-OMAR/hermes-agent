...
export interface tui_gateway_contracts_herwork_device_usage_ProjectIdParams {
  profile?: string | null
  id?: string | null
}
...
export interface ProjectWorkflowParams {
  profile?: string | null
  id?: string | null
...
export interface ReferenceCitationParams {
  profile?: string | null
  id?: string | null
...
/** A row of ``project_actions`` (``SELECT *``), with its ``citation`` joined in by ``list_actions``. */
...
export interface ActionDraftParams {
  profile?: string | null
  id?: string | null
  file_id?: string | null
}
...
export interface ResultIdParams {
  profile?: string | null
  result_id?: string | null
}
export interface ResultVersionsResult {
...
export interface ResultVersionResult {
...
/** ``preview_version``. ``kind`` says which of the three bodies is present: none for ``unsupported`` / ``too_large``, ``text`` for html / svg / text, ``data_url`` for image / pdf. Only a snapshot this profile owns is ever read, never a client path. */
export interface ResultPreviewResult {
  kind: string
  text?: string | null
...
export interface ProjectEditParams {
  ...
  id?: string | null
  name?: string | null
  folders?: ProjectFolderEdit[] | null
}
/** One folder in an edit draft. ``path`` is required; the rest is optional. */
export interface ProjectFolderEdit {
  path: string
  label?: string | null
  is_primary?: boolean
}
/** The saved project, plus every live workspace path that moved with it. A running task is never re-homed under its own tools, so a move refuses while one is running. */
export interface ProjectEditResult {
  project: OpenModel
  relocated_paths?: Record<string, string>
}
/** A result/payload row whose known fields are typed but which the producer may extend (the closed set is owned elsewhere: hermes_state rows, provider inventories). */
export type OpenModel = Record<string, unknown>
export interface DeviceStatusParams {
  session_id?: string | null
}
/** ``methods_device`` answers without touching the device where it can, so a panel can ask on every mount without making an absent phone expensive. */
...
export interface DeviceFrameParams {
  session_id?: string | null
  quality?: number | null
  max_width?: number | null
}
/** A failure is reported as unavailable with its reason rather than as an error: a panel polling this wants to show "the device went away" and keep polling. */
...
/** Fails open: a collection failure answers ``available: false`` with no providers rather than an error, because this panel must never take the window down with it. */
...
export interface tui_gateway_contracts_projects_pets_ProjectIdParams {
  profile?: string | null
  id: string
}
...
/** ``agent/next_moves.py::emit_next_moves`` - the post-turn suggestion pack the composer offers as ghost text, taken with Tab. */
export interface NextMovesOfferPayload {
  moves?: NextMove[]
  session_id: string
  source: string
  turn_id: string
...
/** ``agent/turn_outcome.py::emit_turn_outcome`` - delivered / failed / open for the turn that just settled, carrying the CLIENT turn id so it lands on the row it describes. */
...
export interface RpcMethods {
  /** Subscription usage across every authenticated provider. Local to this fork. */
  'account.usage': { params: ProviderUsageParams; result: ProviderUsageResult }
  ...
  /** One JPEG frame of this session's device, and the size it was captured at. */
  'device.frame': { params: DeviceFrameParams; result: DeviceFrameResult }
  /** Whether this host can show a device at all, and which one. Local to this fork. */
  'device.status': { params: DeviceStatusParams; result: DeviceStatusResult }
  ...
  /** Accept a proposal onto the project's task board. Idempotent once accepted. */
  'projects.actions.accept': { params: ActionIdParams; result: ProjectAction }
  /** Dismiss a proposal without creating a task. */
  'projects.actions.dismiss': { params: ActionIdParams; result: ProjectAction }
  /** Composer text that asks the agent to extract actions from one indexed source. */
  'projects.actions.draft': { params: ActionDraftParams; result: ProjectWorkflowResult }
  /** Edit a pending proposal. Only pending proposals can be edited. */
  'projects.actions.edit': { params: ActionEditParams; result: ProjectAction }
  /** A page of extracted action proposals, newest first. */
  'projects.actions.list': { params: ActionListParams; result: ActionListResult }
  ...
  /** The project's sources and approved results, for the workspace overview. */
  'projects.brief': { params: tui_gateway_contracts_herwork_device_usage_ProjectIdParams; result: ProjectBriefResult }
  ...
  'projects.delete': { params: tui_gateway_contracts_projects_pets_ProjectIdParams; result: ProjectsPayload }
  ...
  /** Rename a project, reconnect a moved folder, add or remove source folders. */
  'projects.edit': { params: ProjectEditParams; result: ProjectEditResult }
  ...
  'projects.get': { params: tui_gateway_contracts_projects_pets_ProjectIdParams; result: ProjectResult }
  ...
  /** One citation by id, with the source version it was taken from. */
  'projects.references.citation': { params: ReferenceCitationParams; result: Citation }
  /** Extract one batch of queued sources; call again while has_more. */
  'projects.references.index': { params: tui_gateway_contracts_herwork_device_usage_ProjectIdParams; result: ReferenceIndexResult }
  /** Re-read the project's source folders and queue what changed. */
  'projects.references.scan': { params: tui_gateway_contracts_herwork_device_usage_ProjectIdParams; result: ReferenceScanResult }
  /** Full-text search across the project's indexed sources. */
  'projects.references.search': { params: ReferenceSearchParams; result: ReferenceSearchResult }
  /** Detach a folder from a project. */
  'projects.remove_folder': { params: ProjectFolderParams; result: ProjectResult }
  /** Snapshot the result's current bytes as a new version. */
  'projects.results.capture': { params: ResultIdParams; result: ResultVersionResult }
  /** A page of captured results, newest report first. */
  'projects.results.list': { params: ResultsListParams; result: ResultsListResult }
  /** Read one verified snapshot for display. */
  'projects.results.preview': { params: VersionIdParams; result: ResultPreviewResult }
  /** Scan recent sessions for delivered results; call again while has_more. */
  'projects.results.refresh': { params: ProfileParams; result: ResultsRefreshResult }
  /** Set a version's review state: unreviewed, approved or changes_requested. */
  'projects.results.review': { params: ResultReviewParams; result: ResultVersionResult }
  /** Every captured version of one result, newest first. */
  'projects.results.versions': { params: ResultIdParams; result: ResultVersionsResult }
  ...
  /** Draft the composer text for a project workflow. Local to this fork. */
  'projects.workflow': { params: ProjectWorkflowParams; result: ProjectWorkflowResult }
...
export const RPC_METHODS = [
  ...
  'profiles.set_asset',
  'project.facts',
  'projects.actions.accept',
  'projects.actions.dismiss',
  'projects.actions.draft',
  ...
  'projects.references.citation',
  'projects.references.index',
  'projects.references.scan',
  'projects.references.search',
  'projects.remove_folder',
  'projects.results.capture',
  'projects.results.list',
  'projects.results.preview',
  'projects.results.refresh',
  'projects.results.review',
...
export interface BackendGatewayEventMap {
  ...
  /** Post-turn suggestions for the composer's ghost text. Local to this fork. */
  'next_moves.offer': NextMovesOfferPayload
  ...
  /** How the finished turn settled, landed on its final assistant row. Local to this fork. */
  'session.outcome': SessionOutcomePayload
