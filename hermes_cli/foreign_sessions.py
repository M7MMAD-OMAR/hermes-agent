"""Import sessions from foreign coding agents (Claude Code, Codex CLI, Kimi Code). Foreign files are
only ever read; imported history must satisfy the provider role-alternation invariant (see
``_merge_turns``)."""
    ...
    for obj in _read_json_lines(path):
        ...
        if otype == "context.append_message":
            ...
            if isinstance(origin, dict) and origin.get("kind") != "user":
                continue
            text = _flatten_blocks(message.get("content"))
            if not text or _WRAPPER_TAG_RE.match(text.lstrip()):
                continue
...
def _list_sessions(source: str, root: Optional[Path]) -> List[ForeignSession]:
    default_root, pattern, recursive, parse, mtime_of = _SOURCES[source]
    ...
    for jsonl in sorted((root.rglob(pattern) if recursive else root.glob(pattern)) if root.is_dir() else ()):
        try:
            # A source may carry its own timestamp (Kimi keeps it in the
            # session's state file); fall back to the file's own mtime.
            mtime = mtime_of(jsonl) if mtime_of is not None else None
            if mtime is None:
...
def gather_foreign_sessions(source: Optional[str] = None, *, claude_root: Optional[Path] = None,
                            codex_root: Optional[Path] = None, kimi_root: Optional[Path] = None,
                            limit: int = 25) -> List[ForeignSession]:
    """List foreign sessions across sources, newest first."""
    sessions = [s for name, root in (("claude", claude_root), ("codex", codex_root), ("kimi", kimi_root))
                if source in (None, name) for s in _list_sessions(name, root)]
