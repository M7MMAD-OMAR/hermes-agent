...
export interface BackendGatewayEventMap {
  ...
  /** `agent/next_moves.py::emit` — the post-turn suggestion pack the composer
   *  offers as ghost text. Local to this fork. */
  'next_moves.offer': Record<string, unknown>
  'session.info': Record<string, unknown>
  /** `agent/turn_outcome.py::emit` — delivered / failed / open for the turn
   *  that just settled, landed on its final assistant row. Local to this fork. */
  'session.outcome': Record<string, unknown>
