...
declare global {
  interface Window {
    hermesDesktop: {
      ...
      // Sbar Rafiq Cloud: one portal login powers discovery + silent per-agent
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Sbar Rafiq Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Sbar Rafiq Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Sbar Rafiq Cloud org (slug or id) to
...
// --- Sbar Rafiq Cloud (cloud-auto-discovery Phase 3) ---
...
// A discovered Sbar Rafiq Cloud agent — the trimmed DTO from NAS GET /api/agents.
