...
import { DEFAULT_TRANSLATIONS, translateNow, translationsFor } from '@/i18n'
...
export function handleStatusEvent(ctx: GatewayEventContext): boolean {
  ...
  if (event.type === 'notification.show') {
    ...
    // native OS notification when Sbar Rafiq is backgrounded; dispatch is gated
  ...
  if (event.type === 'error') {
    const errorMessage = payload?.message || 'Sbar Rafiq reported an error'
    ...
    const card = surface ? errorCardText((translationsFor(getRuntimeI18nLocale()) ?? DEFAULT_TRANSLATIONS).assistant.thread, surface) : null
