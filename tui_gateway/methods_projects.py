...
def _transfer_plan(pdb, conn, params: dict, source_db) -> dict:
    ...
    return project_transfer.plan_transfer(
        ...
        source_conn=conn, source_db=source_db, resolve=git_probe.resolve)


@_projects_method("projects.transfer_plan")
def _(rid, params, pdb, conn) -> dict:
    """What moving this project to another profile would carry. Writes nothing."""
    # The SOURCE is whichever profile the sidebar is showing, not the launch profile:
    # in app-global remote mode those differ, and reading the wrong store would count
    # someone else's conversations.
    with _profile_db(params) as source_db:
        plan = _transfer_plan(pdb, conn, params, source_db)
...
def _(rid, params, pdb, conn) -> dict:
    ...
    # A move retires the donor rows, so the source handle is a writer even though the
    # plan itself only reads.
    with _profile_db(params, writer=True) as source_db:
        plan = _transfer_plan(pdb, conn, params, source_db)
        if plan.get("blockers"):
            raise ValueError("; ".join(plan["blockers"]))
        target_profile = str(plan["target_profile"])
        # The registry hands back the SAME handle when this gateway already serves the
        # target profile (the multiplexer case), so a transfer never opens a second
        # writer on a store its own process is already writing.
