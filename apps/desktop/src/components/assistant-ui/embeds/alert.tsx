...
const ALERT_STYLES: Record<AlertType, AlertStyle> = {
  caution: { accent: 'text-(--ui-danger)', icon: AlertTriangle, label: 'Caution' },
  important: { accent: 'text-violet-600 dark:text-violet-400', icon: AlertCircle, label: 'Important' },
  note: { accent: 'text-(--ui-info)', icon: Info, label: 'Note' },
  tip: { accent: 'text-(--ui-success)', icon: Zap, label: 'Tip' },
  warning: { accent: 'text-(--ui-warning)', icon: AlertTriangle, label: 'Warning' }
}
...
export function MarkdownAlert({ children, type }: { children: ReactNode; type: AlertType }) {
  ...
  return (
    <div
      className="my-2 rounded-lg bg-(--ui-widget-surface-background) px-3 py-2 [&>*:first-child]:mt-0 [&>*:last-child]:mb-0"
